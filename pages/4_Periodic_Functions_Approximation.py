...
st.title("Periodic Functions Approximation")
N = st.slider("Number of points (N)", min_value=2, max_value=100, value=50)
...
compare_dataframe = pd.DataFrame({
    'x': x_points,
    'Original Function': y_points,
    'Approximated Function': y_points_app
})

fig = px.line(compare_dataframe, x='x', y=['Original Function', 'Approximated Function'])
...
compare_f2_dataframe = pd.DataFrame({
    'x': x_points,
    'Original Function': y_points,
    'Approximated Function': y_points_app
})

fig = px.line(compare_f2_dataframe, x='x', y=['Original Function', 'Approximated Function'])
st.plotly_chart(fig, use_container_width=True)
...
compare_f3_dataframe = pd.DataFrame({
    'x': x_points,
    'Original Function': y_points,
    'Approximated Function': y_points_app
})

fig = px.line(compare_f3_dataframe, x='x', y=['Original Function', 'Approximated Function'])
st.plotly_chart(fig, use_container_width=True)
